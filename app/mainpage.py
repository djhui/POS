...
def main():
    if request.method == 'POST':
        ...
        try:
            ...
            Recashes = sales = 0
        try:
            transfee = db.session.query(func.sum(Sales.Aprice)).filter("orderdate like <=:date1","orderdate like >=:date2", "transportation not like :transportation", "offset=:offset").params(date1=date1,date2=date2, transportation=u"%到付%", offset=0).first()
            if transfee[0] == None:
                Aprice = 0
            else: Aprice = transfee[0]
        except:Aprice = 0
